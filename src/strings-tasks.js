...
function countVowels(str) {
  const newStr = str.toLowerCase().replace(/[aeyuio]/gi, '');
...
function reverseWords(str) {
  ...
  let arrRev = [];
  arr.forEach((item) => {
    const itemNew = item.split('').reverse().join('');
    arrRev = [...arrRev, itemNew];
  });
  return arrRev.join(' ');
}
...
function invertCase(str) {
  ...
  arrayFromStr.forEach((item, index) => {
    if (item === item.toUpperCase()) {
      arrayFromStr[index] = item.toLowerCase();
    } else {
      arrayFromStr[index] = item.toUpperCase();
    }
  });
...
function encodeToRot13(str) {
  const arrayIn = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const arrayOut = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  const arr = str.split('');
  arr.forEach((item, index) => {
    for (let i = 0; i < 52; i += 1) {
      if (item === arrayIn[i]) {
        arr[index] = arrayOut[i];
      }
    }
  });
  return arr.join('');
